...
class ActionWidget(QDialog):
    ...
    def __init__(self, parent: QWidget, level_count: int, current_level: int,
        ...
        self.interface.actionCB.setCurrentText(node_type.name)
    ...
    def run(self) -> dict:
        ...
        if self.exec_():
            ...
            node_attr["action_type"] = Action_Type[self.interface.actionCB.currentText()]
    ...
    def verify(self) -> None:
        ...
        '''
        action_type: str = self.interface.actionCB.currentText()
        value: str = self.interface.valueLE.text()

        # If user input is not an action type, then show error message
        if not hasattr(Action_Type, action_type):
            QMessageBox.warning(self, "Alert", "Invalid Action option.")
            return None

        if Action_Type[action_type] == Action_Type.Click and validHTMLTag(value):
            # Accept if Action type is 'Click' and is valid HTML tag
            self.accept()
        elif Action_Type[action_type] == Action_Type.Time and (value.isdigit()
                                                               and 0 < int(value) < 21):
